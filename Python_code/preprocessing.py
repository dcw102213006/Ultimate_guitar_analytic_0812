...
def smooth_key(chord_progression,tokey):
    '''
    ...
    chord_progression_key=key_estimate(chord_progression)['estimate_key']
    estimate_key=key_profile[keyName.index(chord_progression_key)]
    ...
    for i in range(len(estimate_progression)):
        ...
        if sp[0] is not "N":
            if "b" in sp[0]:
                ...
                if sp[0] not in estimate_key:estimate_progression[i] = "N"
            else:
                #print sp[0]
                try:
                    estimate_progression[i] = tokey[estimate_key.index(sp[0])]+":"+sp[1]
